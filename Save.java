...
public class Save {
	...
	static void normalCount(JTextField start, JComboBox save) 
	{
		...
		hopeSave=((String) save.getSelectedItem()).trim().isEmpty() ? 0 :Integer.parseInt((String)(save.getSelectedItem()));
...
//		save30%、50%、70%，spend25%、20%、12.5%，life25%、20%、12.5%，fun20%、10%、5%
		return "自動幫您分配現有資金："+Save.getStart()
				+ "元\n存錢方式為：\n預計存錢金額："+
				Save.getSave()+"元\n固定花費："+Save.getSpend()+"元\n生活開銷為："+Save.getLife()+"元\n預計娛樂花費有"+Save.getFun()+"元";
	}
	...
	static String showOverhead()
	{
		...
		return "極限挑戰模式將會以存錢為目的，所以接下來"
				+Save.getStart()+ "元\n分配為：\n存入金額："+Save.getSave()+"元\n固定花費壓縮在："+Save.getSpend()
